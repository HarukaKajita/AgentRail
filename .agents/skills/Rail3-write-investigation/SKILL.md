...
description: work/<task-id>/investigation.md を作成・更新し、仮説・調査方法（観測方法）・調査結果（観測結果）・結論を事実ベースで確定する。判断不能点は確認質問2-4件と blocked 提案で扱う。
---
...
1. 調査対象と仮説を明確化する。
2. 調査方法（観測方法）を具体化し、確認対象ファイルやコマンドを定義する。
3. 調査結果（観測結果）を「事実」と「推測」に分離して記録する。
