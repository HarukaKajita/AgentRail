...
- 理解ポイント:
  - Wave 3: docs品質KPI閾値定義 の実行順序は wave 計画と depends_on に従う。

## 1. 調査対象 [空欄禁止]

- 課題: 更新遅延/導線整合/網羅率の KPI 指標と暫定閾値を定義する。
...
- 事実:
  - Wave 3: docs品質KPI閾値定義 は wave 計画の分割単位として必要。
  - depends_on は 未解決（2026-02-20__wave2-enforce-doc-quality-fail-mode[planned], 2026-02-20__wave2-align-ci-runbook-with-doc-quality-gates[planned]）。
- 推測:
...
- Wave 3: docs品質KPI閾値定義 を単独タスクとして起票し、wave 依存順序を維持して進行する。
