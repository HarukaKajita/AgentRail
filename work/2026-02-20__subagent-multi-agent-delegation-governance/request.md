...
- 品質懸念を受けて方針変更:
  - `request / investigation / spec` までを単一 subagent/multi_agent に任せる。
  - 親が再検討して問題ない場合のみ次工程とコミットを許可する。

## 要望分析

- 直要求:
  - 委譲範囲を `request / investigation / spec` に限定して仕様化する。
  - 3工程を単一委譲エージェントで処理する契約を定義する。
...
1. 3工程限定委譲（推奨）:
   - `request / investigation / spec` を単一委譲エージェントへ任せる。
2. 全工程強制委譲:
...
2. `spec.md` に `request / investigation / spec` 限定委譲と親再検討ゲートが検証可能な形で定義される。
