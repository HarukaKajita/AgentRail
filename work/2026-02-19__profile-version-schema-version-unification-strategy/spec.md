...
- **検証項目**: 正常系 1 ケースと異常系 2 ケースが意図どおり判定される
- **合格基準**: 現行 profile は PASS、`version` 混入と旧 `schema_version` は FAIL
- Unit Test-1:
...
- Integration Test-1:
  ...
  - **合格基準**: `PASS` を返し、必須成果物整合が満たされる。

### 5.3 回帰テスト (Regression Test) (必須)

- **対象**: repository 全体の state/docs 整合（`state-validate`、`docs-indexer check`）
- **検証項目**: 本変更が既存タスク管理と docs 導線に回帰を起こさない
- **合格基準**: `state-validate` と `docs-indexer check` がともに `PASS`
- Regression Test-1:
...
- **期待される結果**: 破壊的統合の実装・検証・記録がすべて完了している。
