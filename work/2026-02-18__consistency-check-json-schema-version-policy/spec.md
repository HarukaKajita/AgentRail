...
- **検証項目**: single/multi 両モードで `schema_version` が必須項目として出力される
- **合格基準**: `ConvertFrom-Json` 後に `schema_version` が空でなく、single/multi で一致する

### 5.2 結合テスト (Integration Test) (必須)

- **対象**: `-OutputFormat json -OutputFile <path>` 実行
- **検証項目**: 標準出力 JSON とファイル保存 JSON の双方に `schema_version` が含まれる
- **合格基準**: 標準出力と保存ファイルの `schema_version` が一致する

### 5.3 回帰テスト (Regression Test) (必須)

- **対象**: 既存 text モード実行（`-TaskId`）
- **検証項目**: `CHECK_RESULT` / failure 行フォーマットと終了コードが従来互換である
- **合格基準**: 正常 task で PASS(0)、異常 task で FAIL(1) の挙動が維持される
...
- **期待される結果**: AC-001〜AC-004 を満たし、text モード回帰がない
