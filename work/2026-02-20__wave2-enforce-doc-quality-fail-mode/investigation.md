...
- 理解ポイント:
  - Wave 2: docs品質チェック fail 昇格 の実行順序は wave 計画と depends_on に従う。

## 1. 調査対象 [空欄禁止]

- 課題: warning モードの結果を踏まえ、fail モードへ昇格する。
...
- 事実:
  - Wave 2: docs品質チェック fail 昇格 は wave 計画の分割単位として必要。
  - depends_on は 未解決（2026-02-20__wave2-implement-doc-quality-warning-mode[planned]）。
- 推測:
...
- Wave 2: docs品質チェック fail 昇格 を単独タスクとして起票し、wave 依存順序を維持して進行する。
