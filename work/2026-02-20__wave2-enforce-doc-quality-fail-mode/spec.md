...
- **検証項目**: fail mode 指定が対象 task 経路へ組み込まれている。
- **合格基準**: workflow 内で次が確認できる。
...
- **検証項目**: warning 段階で導入した mode が fail 昇格で正常に動作する。
- **合格基準**:
...
- **対象**: 全体チェック運用。
- **検証項目**: `-AllTasks` は warning 維持で既存運用を壊さない。
- **合格基準**:
...
- **期待される結果**: fail mode の強制と段階運用の両立を確認できる。
