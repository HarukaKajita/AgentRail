...
- 観点: include/exclude/seed の解決結果が期待どおりになる
- 合格条件: `-DryRun` で copy 対象件数と出力先が期待どおりに表示される

### 5.2 Integration Test (Integration Test / 結合テスト) [空欄禁止]

- 対象: `framework.runtime.manifest.yaml` と `tools/runtime/export-runtime.ps1`
- 観点: apply で dist/runtime が生成され、check で同期状態を検証できる
- 合格条件: apply 実行後の check が PASS する
...
- 手順: `-Mode apply` 実行後に `-Mode check` と `-DryRun` を順次実行する
