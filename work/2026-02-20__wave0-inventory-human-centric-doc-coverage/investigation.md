# Investigation: 2026-02-20__wave0-inventory-human-centric-doc-coverage
...
- 理解ポイント:
  - Wave 0: must対象の資料棚卸しと欠落マップ作成 の実行順序は wave 計画と depends_on に従う。

## 1. 調査対象 (Investigation Target) (必須)

- 課題: must対象資料の棚卸しと欠落カテゴリ（目的/使い方/仕組み/実装/関連）の可視化を行う。
...
- 事実:
  - Wave 0: must対象の資料棚卸しと欠落マップ作成 は wave 計画の分割単位として必要。
  - depends_on は 解決済み。
- 推測:
...
- Wave 0: must対象の資料棚卸しと欠落マップ作成 を単独タスクとして起票し、wave 依存順序を維持して進行する。
...
- spec: `work/2026-02-20__wave0-inventory-human-centric-doc-coverage/spec.md`
