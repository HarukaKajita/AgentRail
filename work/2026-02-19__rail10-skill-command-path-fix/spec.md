...
- **合格基準**: 検索結果で禁止文字列が0件、想定コマンド文字列が1件以上。
...
- **合格基準**: 終了コード0かつ出力形式が要件を満たす。

### 5.3 回帰テスト (Regression Test) (必須)

- **対象**: task 成果物整合 (`tools/consistency-check/check.ps1`)
- **検証項目**: 本タスク文書更新後も必須成果物整合ルールを満たす。
- **合格基準**: `check.ps1 -TaskId 2026-02-19__rail10-skill-command-path-fix` が PASS。
...
- **期待される結果**: 受入条件 AC-001〜AC-005 を満たす。
