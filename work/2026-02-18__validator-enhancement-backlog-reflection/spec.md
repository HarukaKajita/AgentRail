...
- **検証項目**: 各 backlog 項目が `item_id`, `source`, `priority`, `status`, `proposal` を持つ
- **合格基準**: 記載項目が必須属性を満たし、空欄がない

### 5.2 結合テスト (Integration Test) (必須)

- **対象**: docs 間リンク整合
- **検証項目**: high-priority backlog と docs index の双方から新規 backlog 資料へ到達できる
- **合格基準**: `tools/docs-indexer/index.ps1` 実行後もリンク不整合がない

### 5.3 回帰テスト (Regression Test) (必須)

- **対象**: validator 実行フロー
- **検証項目**: docs-only 変更で profile/state validator の実行結果に影響がない
- **合格基準**: `tools/profile-validate/validate.ps1` と `tools/state-validate/validate.ps1` が既存と同様に PASS
