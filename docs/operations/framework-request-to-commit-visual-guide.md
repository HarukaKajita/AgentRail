...
6. 着手前依存確認: `state.json` の `depends_on` を確認し、未解決依存があれば先行タスクへ切り替える。
7. 実装: コード/ドキュメントを変更する。
8. テスト: 実行コマンドで期待結果を検証する。
9. レビュー: `review.md` に AC 判定・テスト結果・Process Findings を記録する。
10. 資料更新: 変更した docs を更新し、`docs/INDEX.md` へ導線を追加する。
11. 記憶更新: `MEMORY.md` と `state.json` を最新化する。
12. コミット: 変更を意味のある単位でコミットする。
